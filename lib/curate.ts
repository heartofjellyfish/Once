...
WHAT TO OUTPUT:
- passed_filter: true when the reader test (a+b+c) passes, per the
  SPIRIT above. Format — still scene, focal event with backstory,
  chain around one object — does not decide; the reader-side
  question does. When in doubt, PASS — a human editor reviews every
  queued item and will catch false positives.
